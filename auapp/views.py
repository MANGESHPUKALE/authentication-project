		...
		if request.method =="POST":
			...
			if pw1 == pw2:
				try :
					...
					return render(request, "usignup.html", {"msg":"User already register"})
				except User.DoesNotExist:
					usr = User.objects.create_user(username=un, password = pw1)
					usr.save()
					return redirect("uhome")
			else:
				return render(request, "usignup.html", {"msg":"Password does not match"})
		else:
	...
	if request.user.is_authenticated:
		if request.method =="POST":
			...
			if pw1 == pw2:
				...
				return render(request, "ucp.html", {"msg":"Password does not match"})
		else : 
